...
from raganything import QueryMixin
from raganything import ProcessorMixin
from raganything import BatchMixin
from raganything.utils import get_processor_supports
from raganything import MineruParser, DoclingParser

# Import specialized processors
from raganything import ImageModalProcessor, TableModalProcessor, EquationModalProcessor
from raganything import GenericModalProcessor,ContextExtractor, ContextConfig
...
class RAGAnything(QueryMixin, ProcessorMixin, BatchMixin):
    ...
    Include:
        - NOTE Core Components:
            ...
            - llm_model_func:                   Optional LLM model function for text analysis.
            - vision_model_func:                Optional vision model function for image analysis.
            - embedding_func:                   Optional embedding function for text vectorization.
            - config:                           Optional Configuration object, if None will create with environment variables.
            
        - NOTE LightRAG Configuration:
            - lightrag_kwargs:
        ...
        - NOTE Internal State:
            - modal_processors:                 Dictionary of multimodal processors.
            - context_extractor:                Context extractor for providing surrounding content to modal processors.
            - parse_cache:                      Parse result cache storage using LightRAG KV storage.
            - _parser_installation_checked      Flag to track if parser installation has been checked.
    """
    ...
    def get_config_info(self) -> Dict[str, Any]:
        ...
        if self.config is None:
            raise ValueError("RAGAnything config is not initialized")
    ...
    def _initialize_processors(self):
        ...
        if self.config is None:
            raise ValueError("RAGAnything config must be initialized before creating processors")
                    ...
                    if self.parse_cache is None:
                        ...
                        self.parse_cache = (
                            self.lightrag.key_string_value_json_storage_cls(
                                ...
                                embedding_func=self.embedding_func,  # type: ignore
                            )
                        )
