...
Provides async and sync query helpers for both text-only and multimodal query
flows.
...
from .utils import get_processor_for_type, encode_image_to_base64, validate_image_file
...
class QueryMixin:
    ...
    Note:
    ---
    Query mixin for RAGAnything.
    This is a mixin. The host class should provide `lightrag`, `logger`, and
    model functions (`llm_model_func`, `vision_model_func`, etc.).
    ...
    def _generate_multimodal_cache_key(self, 
                                       query: str,
        ...
        **kwargs:
        ...
        Examples:
        ---
        >>> # key = self._generate_multimodal_cache_key("hello", [], "mix")
        """
    ...
    async def aquery(self, query: str, 
        ...
        **kwargs: Other query parameters, will be passed to QueryParam
        ...
        elif vlm_enhanced and (not hasattr(self, "vision_model_func") or not self.vision_model_func):
            self.logger.warning(
        ...
        result = await lightrag.aquery(query, param=query_param, system_prompt=system_prompt)

        if not isinstance(result, str):
            # LightRAG can return an async stream for some configurations.
            # Keep original behavior by forcing a concrete string for this API.
            result = "".join([chunk async for chunk in cast(AsyncIterator[str], result)])

        self.logger.info("Text query completed")
        return result


    async def aquery_with_multimodal(self,
                                     query: str,
        ...
        Note:
        ---
        Multimodal query combining the base text query with additional structured
        content (image/table/equation/etc.).
        ...
        query:
            Base query text.
        multimodal_content:
            Optional list of multimodal content dictionaries.
        mode:
            Query mode.
        **kwargs:
        ...
        Examples:
        ---
        >>> # result = await rag.aquery_with_multimodal("What's here?", [{"type":"image","img_path":"./a.png"}])
        """
    ...
    async def aquery_vlm_enhanced(self,
                                  query: str,
                                  mode: QueryMode = "mix",
                                  system_prompt: Optional[str] = None,
                                  **kwargs: Any,
        ...
        raw_prompt = await lightrag.aquery(query, param=query_param)

        if not isinstance(raw_prompt, str):
            raw_prompt = "".join(
                [chunk async for chunk in cast(AsyncIterator[str], raw_prompt)]
            )
        ...
        if not images_found:
            ...
            query_param = QueryParam(mode=mode, **kwargs)
            fallback = await lightrag.aquery(
                query, param=query_param, system_prompt=system_prompt
            )
            if isinstance(fallback, str):
                return fallback
            return "".join([chunk async for chunk in cast(AsyncIterator[str], fallback)])
        ...
        messages = self._build_vlm_messages_with_images(
            enhanced_prompt, query, system_prompt or ""
        )
    ...
    async def _process_multimodal_query_content(self, 
                                                base_query: str, 
    ...
    async def _generate_query_content_description(self, 
                                                  processor, 
    ...
    async def _describe_generic_for_query(self, processor, content: Dict[str, Any], content_type: str) -> str:
    ...
    def _build_vlm_messages_with_images(self, 
                                        enhanced_prompt: str, 
                                        user_query: str, 
    ...
    async def _call_vlm_with_multimodal_content(self, messages: List[Dict]) -> str:
        ...
        try:
            if self.vision_model_func is None:
                raise ValueError("vision_model_func is not configured")
    ...
    def query(self, query: str, mode: QueryMode = "mix", **kwargs: Any) -> str:
        """
    ...
    def query_with_multimodal(self,
                              query: str,
                              multimodal_content: Optional[List[Dict[str, Any]]] = None,
                              mode: QueryMode = "mix",
                              **kwargs: Any,
