"""Batch processing utilities for RAGAnything.

This module provides batch operations for parsing documents and optionally
running the full RAG insertion pipeline.
"""
...
from .parsers.batch_parser import BatchParser, BatchProcessingResult
...
class BatchMixin:
    ...
    Note:
    ---
    Batch mixin for RAGAnything.
    This is a mixin. The host class is expected to provide:

    - `config`: :class:`~RAGAnythingConfig`
    - `logger`: :class:`logging.Logger`
    - `_ensure_lightrag_initialized()`: async initializer
    - `process_document_complete(file_path, **kwargs)`: async per-file pipeline
    """
    ...
    async def process_folder_complete(self, folder_path: str,
        ...
        Note:
        ---
        Process all supported files in a folder using the full pipeline.

        This method finds files by extension under `folder_path`, then runs
        `process_document_complete` for each file with a bounded concurrency.
        ---
        Args:
        ---
        folder_path:
        ...
        output_dir:
            Directory for parsed outputs (optional).
        parse_method:
            Parsing method to use (optional).
        display_stats:
            Whether to display statistics (optional).
        split_by_character:
            Character to split by (optional).
        split_by_character_only:
            Whether to split only by character (optional).
        file_extensions:
            List of file extensions to process (optional).
        recursive:
            Whether to process folders recursively (optional).
        max_workers:
            Maximum number of workers for concurrent processing (optional).

        ---
        Examples:
        ---
        >>> # async usage
        >>> # await rag.process_folder_complete("./data", recursive=True)
        """
        ...
        tasks: List[asyncio.Task[tuple[bool, str, Optional[str]]]] = []
        ...
        for result in results:
            # asyncio.gather with return_exceptions=True can yield either a tuple
            # (success, file_path, error) or an exception object.
            if isinstance(result, BaseException):
                failed_files.append(("unknown", str(result)))
                continue

            success, file_path_str, error = result
            if success:
                successful_files.append(file_path_str)
            else:
                failed_files.append((file_path_str, error or "unknown error"))
        ...
        if display_stats:
            ...
            if failed_files:
                ...
                for file_path_str, error in failed_files:
                    self.logger.warning(f"  - {file_path_str}: {error}")
    ...
    def process_documents_batch(self, file_paths: List[str],
                                ...
                                **kwargs: Any) -> BatchProcessingResult:
        """
        ---
        Note:
        ---
        Process multiple documents in batch using :class:`BatchParser`.

        This performs parsing only (no RAG insertion). It is a synchronous
        wrapper around :meth:`BatchParser.process_batch`.
        ...
        **kwargs:
        ...
        Examples:
        ---
        >>> # result = rag.process_documents_batch(["a.pdf", "b.pdf"])
        >>> # print(result.successful_files)
        """
    ...
    async def process_documents_batch_async(self, file_paths: List[str],
                                            output_dir: Optional[str] = None,
                                            parse_method: Optional[str] = None,
                                            max_workers: Optional[int] = None,
                                            recursive: Optional[bool] = None,
        ...
        Note:
        ---
        Asynchronously parse multiple documents using :class:`BatchParser`.
        ...
        **kwargs:
        ...
        ---
        >>> # async usage
        >>> # result = await rag.process_documents_batch_async(["./data"])
    ...
    def get_supported_file_extensions(self) -> List[str]:
        ...
        Note:
        ---
        Get supported file extensions for the configured parser.
    ...
    def filter_supported_files(self, 
                               file_paths: List[str], 
                               recursive: Optional[bool] = None
                               ) -> List[str]:
        """
        ...
        ---
        file_paths:
            List of file paths or directories to filter.
        recursive:
            Whether to process directories recursively.
        ...
        Examples:
        ---
        >>> # files = rag.filter_supported_files(["./data", "a.pdf"], recursive=True)
        >>> # print(files)
        """
        if recursive is None:
            recursive = self.config.recursive_folder_processing
    ...
    async def process_documents_with_rag_batch(self, file_paths: List[str],
                                               output_dir: Optional[str] = None,
                                               parse_method: Optional[str] = None,
                                               max_workers: Optional[int] = None,
                                               recursive: Optional[bool] = None,
        ...
        Note:
        ---
        Parse documents in batch and then run the RAG insertion pipeline.

        The method performs:

        1) Parse all documents using batch processing
        2) For successfully parsed documents, run `process_document_complete`
        ...
        show_progress:
            Whether to show progress bar.
        **kwargs:
            Additional arguments passed to the parser and/or RAG pipeline.

        ---
        Examples:
        ---
        >>> # async usage
        >>> # result = await rag.process_documents_with_rag_batch(["./data"]) 
        >>> # print(result["successful_rag_files"]) 
        """
