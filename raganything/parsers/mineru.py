...
class MineruParser(Parser):
    ...
    @classmethod
    def __run_mineru_command(
        cls, input_path: Union[str, Path], output_dir: Union[str, Path], 
        method: str = "auto", lang: Optional[str] = None, backend: Optional[str] = None, 
        start_page: Optional[int] = None, end_page: Optional[int] = None,
        formula: bool = True, table: bool = True, 
        device: Optional[str] = None, source: Optional[str] = None, vlm_url: Optional[str] = None,
    ) -> None:
        """
        ---
        Note:
        ...
        try:
            ...
            # Start subprocess
            # print(f"Starting mineru process with command: {' '.join(cmd)}")
            process = subprocess.Popen(cmd, **subprocess_kwargs)
            # print(f"Mineru process started with PID: {process.pid}")
            # Create queues for stdout and stderr
            stdout_queue = Queue()
            stderr_queue = Queue()
            # print(f"Starting threads to read output")
            ...
            # print(f"Mineru process finished with return code: {return_code}")
    ...
    def parse_pdf(self, pdf_path: Union[str, Path], output_dir: Optional[str] = None,
                  method: str = "auto", lang: Optional[str] = None, **kwargs) -> List[Dict[str, Any]]:
        """
        ---
        Note:
        ---
        ...
        **kwargs: Additional parameters for mineru command
        ...
        try:
            ...
            content_list, _ = self.__read_output_files(base_output_dir, name_without_suff, method=method)
            return content_list
    ...
    def parse_image(self, image_path: Union[str, Path], output_dir: Optional[str] = None,
                    lang: Optional[str] = None, **kwargs) -> List[Dict[str, Any]]:
        """
        ---
        Note:
        ---
        Parse image document using MinerU 2.0
        MinerU 2.0 natively supports .png, .jpeg, .jpg formats.
        ...
        try:
            ...
            try:
                ...
                content_list, _ = self.__read_output_files(
    ...
    def parse_office_doc(self, doc_path: Union[str, Path], output_dir: Optional[str] = None,
                         lang: Optional[str] = None, **kwargs) -> List[Dict[str, Any]]:
        """
    ...
    def parse_text_file(self, text_path: Union[str, Path], output_dir: Optional[str] = None, 
                        lang: Optional[str] = None, **kwargs) -> List[Dict[str, Any]]:
        """
    ...
    def parse_document(self, file_path: Union[str, Path], output_dir: Optional[str] = None,
                       method: str = "auto", lang: Optional[str] = None, **kwargs) -> List[Dict[str, Any]]:
        """
...
def test_main():
    ...
    mineru_parser.check_installation()
    print("MinerU 2.0 is installed and ready to use.")
    
    """
    Test 1: test_pdf(mineru_parser)
    """
    pdf_path = "../../test/samples/pdf/Very_Deep_Convolutional_Networks_for_Large-Scale_Image_Recognition.pdf"
    output_dir = "../../test/samples/pdf/output"
    mineru_parser.parse_pdf(pdf_path, output_dir, lang="en", method="auto")
    print("PDF parsing test completed successfully.")
    
    
    """
    Test 2: test_img(mineru_parser)
    """
    img_path = "../../test/samples/imgs/01.png"
    output_dir = "../../test/samples/imgs/output"
    mineru_parser.parse_image(img_path, output_dir, lang="en")
    print("Image parsing test completed successfully.")
    
    
    """
    Test 3: test_offic(mineru_parser)
    """
    office_path = "../../test/samples/office/sample_office.docx"
    output_dir = "../../test/samples/office/output"
    mineru_parser.parse_office_doc(office_path, output_dir, lang="en", method="auto")
    print("Office document parsing test completed successfully.")
    
    
    """
    Test 4: test_text(mineru_parser)
    """
    text_path = "../../test/samples/text/sample_text.txt"
    output_dir = "../../test/samples/text/output"
    mineru_parser.parse_text_file(text_path, output_dir, lang="en", method="auto")
    print("Text parsing test completed successfully.")
