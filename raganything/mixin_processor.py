...
from .utils.base import DocStatus
from .parsers import MineruParser, DoclingParser, MineruExecutionError
from .processors import ContextExtractor, BaseModalProcessor
from .utils import separate_content,insert_text_content,insert_text_content_with_multimodal_content,get_processor_for_type
...
class ProcessorMixin:
    ...
    llm_model_func: Optional[Callable[..., Any]]
    vision_model_func: Optional[Callable[..., Any]]
    embedding_model_func: Optional[Callable[..., Any]]
    config: RAGAnythingConfig
    modal_processors: Dict[str, Any]
    ContextExtractor: Optional[ContextExtractor]
    parse_cache: Optional[Any]
    _parser_installation_checked: bool
    logger: Logger

    async def _ensure_lightrag_initialized(self):...
    def set_content_source_for_context(self, content_list, content_format): ...
    ...
    def _get_file_reference(self, file_path: str) -> str:
        ...
        if self.config.use_full_path:
            ...
            return os.path.basename(file_path) # get "file.pdf" from "./dir1/file.pdf"


    def _generate_cache_key(self, file_path: Path, parse_method: Optional[str] = None, **kwargs) -> str:
        """
        ---
        Note:
        ---
        Generate cache key based on file path and parsing configuration.
        Since parsing the same file would cost the same, we can use the same cache key to avoid redundant processing.
        ...
        **kwargs: Additional parser parameters
        ...
        mtime = file_path.stat().st_mtime # Get file modification time

        config_dict = { # Create configuration dict for cache key
        ...
        relevant_kwargs = { # Add relevant kwargs to config
            k: v
            for k, v in kwargs.items()
            if k in ["lang", "device", "start_page", "end_page", "formula", "table", "backend","source",]
        }
    ...
    def _generate_content_based_doc_id(self, content_list: List[Dict[str, Any]]) -> str:
        ...
        content_hash_data = [] # Extract key content for ID generation
    ...
    async def _get_cached_result(self, cache_key: str, file_path: Path, parse_method: Optional[str] = None, 
                                 **kwargs) -> tuple[List[Dict[str, Any]], str] | None:
        """
        ---
        Note:
        ---
        Get cached parsing result if available and valid.
        One cached key is for each unique combination of input parameters.
        ...
        **kwargs: Additional parser parameters
        ...
        try:
            ...
            relevant_kwargs = { # Add relevant kwargs to current config
                k: v
                for k, v in kwargs.items()
                if k in ["lang", "device", "start_page", "end_page", "formula", "table", "backend", "source",]
            }
    ...
    async def _store_cached_result(self, cache_key: str, content_list: List[Dict[str, Any]], doc_id: str,
                                   file_path: Path, parse_method: Optional[str] = None, **kwargs) -> None:
        """
        ---
        Note:
        ---
    ...
    async def parse_document(self, file_path: Union[str, Path], output_dir: str = "", parse_method: str = "default",
                             display_stats: bool = False, **kwargs) -> tuple[List[Dict[str, Any]], str]:
        """
        ---
        Note:
        ---
        ...
        **kwargs: Additional parameters for parser (e.g., lang, device, start_page, end_page, formula, table, backend, source)
        ...
        try:
            ...
            if ext in [".pdf"]:
                ...
                content_list = await asyncio.to_thread(
                    doc_parser.parse_document,  # type: ignore
                    file_path=file_path,
                    output_dir=output_dir,
                    method=parse_method,
                    **kwargs,
    ...
    async def _process_multimodal_content(self, multimodal_items: List[Dict[str, Any]], file_path: str,
                                          doc_id: str, pipeline_status: Optional[Any] = None, 
        ...
        try:
            if not self.lightrag is None:
                existing_doc_status = await self.lightrag.doc_status.get_by_id(doc_id)
            else:
                self.logger.warning("LightRAG instance is not available, skipping document status check")
                existing_doc_status = None
    ...
    async def _process_multimodal_content_individual(self, multimodal_items: List[Dict[str, Any]], 
                                                     file_path: str, doc_id: str):
        """
        ---
        Note:
        ---
        ...
        if self.lightrag is None:
            raise ValueError("LightRAG instance is not available")
        else:
            self.logger.info("LightRAG instance is available")
    ...
    async def _process_multimodal_content_batch_type_aware(self, multimodal_items: List[Dict[str, Any]], 
                                                           file_path: str, doc_id: str):
        """
        ---
        Note:
        ...
        async def process_single_item_with_correct_processor(item: Dict[str, Any], index: int, 
                                                             file_path: str):
            """
            ---
            Note:
    ...
    def _convert_to_lightrag_chunks_type_aware(self, multimodal_data_list: List[Dict[str, Any]], 
                                               file_path: str, doc_id: str) -> Dict[str, Any]:
        """
        ---
        Note:
        ---
        ...
        for data in multimodal_data_list:
            ...
            # Calculate tokens
            if self.lightrag.tokenizer is None:
                raise ValueError("LightRAG tokenizer is not initialized")
    ...
    def _apply_chunk_template(self, content_type: str, original_item: Dict[str, Any], 
                              description: str) -> str:
        """
        ---
        Note:
        ...
        """
        from raganything.utils.prompt import PROMPTS
    ...
    async def _store_multimodal_main_entities(self, multimodal_data_list: List[Dict[str, Any]],
                                              lightrag_chunks: Dict[str, Any], file_path: str,
                                              doc_id: str = "",):
        """
        ---
        Note:
    ...
    async def _store_multimodal_entities_to_full_entities(self, entities_to_store: Dict[str, Any], 
                                                          doc_id: str):
        """
        ---
        Note:
    ...
    async def _batch_extract_entities_lightrag_style_type_aware(
        self, lightrag_chunks: Dict[str, Any]) -> List[Tuple]:
        """
        ---
        Note:
        ---
    ...
    async def _batch_add_belongs_to_relations_type_aware(
        self, chunk_results: List[Tuple], multimodal_data_list: List[Dict[str, Any]]) -> List[Tuple]:
    ...
    async def _batch_merge_lightrag_style_type_aware(self, enhanced_chunk_results: List[Tuple], 
                                                     file_path: str, doc_id: str = ""):
        """
        ---
        Note:
    ...
    async def _mark_multimodal_processing_complete(self, doc_id: str):
        ...
        In the LightRAG's doc_status collection, this updates the document's status
        to reflect that multimodal processing is complete.
    ...
    async def process_document_complete(self, file_path: str, output_dir: str = "", parse_method: str = "", display_stats: bool = False, 
                                        split_by_character: str | None = None, split_by_character_only: bool = False, 
                                        doc_id: str | None = None, file_name: str | None = None, **kwargs,):
        """
        ---
        Note:
        ---
    ...
    async def process_document_complete_lightrag_api( self, file_path: str, output_dir: str = "", parse_method: str = "", 
                                                     display_stats: bool = False, split_by_character: str | None = None, 
                                                     split_by_character_only: bool = False, doc_id: str | None = None, 
                                                     scheme_name: str | None = None, parser: str | None = None, **kwargs,):
        """
        ...
        try:
            ...
            if result is None:
                raise ValueError("LightRAG initialization failed")
            ...
            if pipeline_status_lock is None:
                raise ValueError("Pipeline status lock is not initialized")
            
            async with pipeline_status_lock:
                if pipeline_status is None:
                    raise ValueError("Pipeline status is not initialized")
    ...
    async def insert_content_list(self, content_list: List[Dict[str, Any]], file_path: str = "unknown_document", 
                                  split_by_character: str | None = None, split_by_character_only: bool = False,
                                  doc_id: str | None = None, display_stats: bool = False,):
        """
